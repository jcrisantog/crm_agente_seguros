...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        async function loadProduct() {
            try {
                ...
                if (error) {
                    console.error("SDK Error loading product:", JSON.stringify(error, null, 2));
                ...
                if (data) {
                    setFormData({
                        ...
                        has_deductible: data.has_deductible === true,
                    });
                    setRequiredDocs(Array.isArray(data.required_docs_schema) ? data.required_docs_schema : []);
                } else if (!error) {
                    console.error("No data found for product ID:", resolvedParams.id);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error("Error al actualizar catálogo: " + (error.message || "Intenta de nuevo"));
        } finally {
    ...
    if (isLoading) {
        return <div className="p-12 text-center text-muted-foreground">Cargando producto...</div>;
    }
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Editar Tipo de Seguro</h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        Modifica los detalles generales y documentos base.
                    </p>
                </div>
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-sm font-medium leading-none">Nombre del Seguro *</label>
                                <input required name="name" value={formData.name} onChange={handleChange} className="flex h-10 w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus-[ring]" />
                            </div>

                            <div className="space-y-2">
                                <label className="text-sm font-medium leading-none">Descripción Corta</label>
                                <textarea name="description" rows={3} value={formData.description} onChange={handleChange} className="flex w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus-[ring] resize-y" />
                            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2 items-center mt-4">
                            ...
                            />
                            <button type="button" onClick={handleAddDoc} className="h-10 inline-flex items-center justify-center rounded-md bg-secondary px-4 text-sm font-medium text-secondary-foreground hover:bg-secondary/80 border border-border">
                            ...
                            {requiredDocs.length > 0 ? (
                                requiredDocs.map((doc, idx) => (
                                    <div key={idx} className="flex justify-between items-center p-3 rounded-md bg-background border border-border/50 shadow-sm">
                                        ...
                                        <button type="button" onClick={() => handleRemoveDoc(doc)} className="text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 p-1.5 rounded-md transition-colors">
                    ...
                    <div className="flex items-center justify-end gap-4 pt-6 mt-6 border-t border-border">
                        <Link href="/products">
                            <button type="button" className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground">Cancelar</button>
                        </Link>
                        <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center rounded-md bg-primary px-6 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50">
