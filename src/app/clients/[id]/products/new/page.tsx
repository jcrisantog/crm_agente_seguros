...
export default function NewPolicyPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        async function loadData() {
            ...
            setError(null);
            try {
                const [clientRes, productsRes, systemSettingsRes] = await Promise.all([
                ...
                if (clientRes.error) setError("No se pudo encontrar al cliente o el ID es inválido.");
                if (clientRes.data) {
                    setClientName(clientRes.data.full_name);
                    // Pre-fill contratante based on client generic data
                    setContratante(prev => ({ ...prev, nombre: clientRes.data.full_name }));
                }
    ...
    const addCard = () => {
        ...
        setCurrentCard({ no_tarjeta: "", banco: "", tipo_tarjeta: "", is_main: false });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error: insforgeError } = await insforge.database.from("client_products").insert({
                ...
                prima_por_periodo: formData.prima_por_periodo ? parseFloat(formData.prima_por_periodo) : 0,
                comision_por_periodo: formData.comision_por_periodo ? parseFloat(formData.comision_por_periodo) : 0,
            ...
            toast.success("Nueva póliza asignada correctamente");
            ...
            if (error.message && error.message.includes("JWT expired")) {
                ...
                toast.error("Error al registrar póliza: " + (error.message || "Revisa la consola"));
            }
        } finally {
            setIsSubmitting(false);
        }
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12 relative">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">Registro de Póliza Completo</h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        Cliente asociado: <strong className="text-foreground">{error ? "Error" : clientName}</strong>
                    </p>
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="bg-card border border-border shadow-sm rounded-xl p-12 text-center text-red-500 font-semibold">{error}</div>
            ) : (
